...
```{r}
sdf <- left_join(sdf, df, by = c("COUNTRY" = "country"))
```
...
```{r}
full_df <- left_join(sdf, df, by = c("ISO" = "iso_code")) # join the missing countries by the ISO code 

full_df$gender_inequality <- ifelse(is.na(full_df$gender_inequality.x), full_df$gender_inequality.y, full_df$gender_inequality.x) # make the two index columns into one

full_df <- subset(full_df, select = -c(gender_inequality.x, gender_inequality.y, country)) #drop the columns we do not need
...
```{r}
#st_write(full_df, "genderInequalityIndex.gpkg")
```
